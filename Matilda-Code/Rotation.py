...
def create_mask(img, max_r, min_r):
    shape = img.shape
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            ...
            if distance <= max_r and distance >= min_r:
                mask[i, j] = True
            else:
                mask[i, j] = False
...
def find_rotation(image):
    ...
    mask = create_mask(segmented_image, 70, 10)
    img2 = segmented_image * mask

    img2 = median_filter(img2, size=5)
    mask = create_mask(img2, 65, 15)
    img2 = img2 * mask
    plt.imshow(img2)
    plt.show()
...
def rotate_volume(im_list, alpha, center, output_path):
    ...
    for i, im in enumerate(im_list):
        ...
        vol[i, :, :] = f
    vol_part = vol[20,:,:]
    rot_part = scipy.ndimage.rotate(vol_part,alpha, axes =(1,0))
    plt.imshow(rot_part)
    plt.show()
