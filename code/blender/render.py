...
def modify_properties(obj, label, isJellyroll):
    ...
    if isJellyroll:
        ...
        return
    obj.data.materials.append(bpy.data.materials[f'Material{label%len(color_palette())}'])
    return
...
def create_materials(isJellyroll):
    ...
    mat['ShellMaterial'].node_tree.nodes['Principled BSDF'].inputs[21].default_value = 0.75 if isJellyroll else 0.1
    if not isJellyroll:
...
def persp_view_render(persp_path, time, isJellyroll):
    ...
    bpy.ops.object.camera_add(enter_editmode=False, location=location, rotation=(np.pi/4, 0, np.pi/4))
    bpy.context.scene.camera = bpy.context.active_object
...
if __name__ == "__main__":
    _, path, exp, isJellyroll = sys.argv
    modify_engine(isJellyroll)
    create_materials(isJellyroll)
    stl_path, side_path, top_path, persp_path = create_folders(path, exp, isJellyroll)
